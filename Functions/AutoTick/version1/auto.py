...
class Tickter(object):
        ...
        while True:
            try:
                ...
                if res['result_code'] == 0:
                    ...
                    while True:
                        try:
                            response = self.session.post('https://kyfw.12306.cn/otn/login/userLogin',
                                                         data={"_json_att": ""}, headers=headers)
                            response = self.session.post('https://kyfw.12306.cn/passport/web/auth/uamtk',
                                                         data={"appid": "otn"}, headers=headers)
                    ...
                    # 第二页乘车人
                    try:
                        resu = self.session.post('https://kyfw.12306.cn/otn/passengers/query',
    ...
    def order(self, *arg, **kw):
        ...
        while True:
            try:
                res = self.session.post('https://kyfw.12306.cn/otn/leftTicket/submitOrderRequest', data=arg[4],
        ...
        if r['status']:
            ...
            r = self.session.post('https://kyfw.12306.cn/otn/confirmPassenger/checkOrderInfo', data=data,
                                  headers=headers)
            res = r.json()
            print(res)
            if res['data']['submitStatus']:
                ...
                r = self.session.post('https://kyfw.12306.cn/otn/confirmPassenger/getQueueCount', data=data,
                                      headers=headers)
                r = r.json()
                print(r)
                # time.sleep()
                if r['status'] and int(r['data']['ticket']) > 1:
                    ...
                    print(data)
                    r = self.session.post('https://kyfw.12306.cn/otn/confirmPassenger/confirmSingleForQueue', data=data,
                                          headers=headers)
                    ...
                    r = self.session.get('https://kyfw.12306.cn/otn/confirmPassenger/queryOrderWaitTime', params=data,
                                         headers=headers)
                    print(r.json())
                    ...
                    if re['status'] and re['data']['queryOrderWaitTimeStatus']:
                        ...
                        r = self.session.post('https://kyfw.12306.cn/otn/confirmPassenger/resultOrderForDcQueue',
    ...
    def _repeat_submit_token(self):
        ...
        ress = self.session.post('https://kyfw.12306.cn/otn/confirmPassenger/initDc', data={"_json_att": ""},
                                 headers=headers)
    ...
    def query(self, task):
        ...
        while True:
            ...
            url = 'https://kyfw.12306.cn/otn/leftTicket/queryZ?leftTicketDTO.train_date=%s&leftTicketDTO.from_station=%s&leftTicketDTO.to_station=%s&purpose_codes=ADULT' % (
                self.date, self.form, self.to)
            try:
            ...
            if result:
                for i in result:
                    ...
                    if ll[28] != '' and ll[28] != '无' and ll[28] != '*':
                        ...
                        while True:
                            try:
                                r = self.session.post('https://kyfw.12306.cn/otn/login/checkUser',
                                                      data={'_json_att': ''}, headers=headers)
                                rr = r.json()
                            except:
                                print(rr.text)
