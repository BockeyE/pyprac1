...
def show_like_interface(screen):
    ...
    background1 = pygame.image.load('timg.jpg').convert()
...
def main():
    ...
    # 添加背景音乐
    # try:
    #     pygame.mixer.music.load('214.mp3')
    ...
    while running:
        ...
        # 设置标题及按钮文本信息
        title('有一个脱单的机会，要不要考虑一下？', screen, scale=[8, 3])
        button('A.好的！', like_pos_x, like_pos_y, like_pos_width, like_pos_height, like_color, screen)
        button('B.我拿你当闺蜜，你居然想睡我！果断拒绝！', unlike_pos_x, unlike_pos_y, unlike_pos_width, unlike_pos_height, unlike_color,
               screen)
        # 设置关闭选项属性
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
